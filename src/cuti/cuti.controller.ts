import { Body, Controller, Post, Req, UseGuards, Get, Query, Param, Patch, UploadedFile, UseInterceptors, HttpException, HttpStatus } from '@nestjs/common';
...
export class CutiController {
    ...
    @UseGuards(AuthGuard())
    createCuti(@UploadedFile() image: BufferedFile, @Body() createCutiDto: CreateCutiDto, @Req() req,): Promise<Cuti> {
    ...
    @Patch('approved/:id')
    @UseGuards(AuthGuard())
    async updateApproved(@Param('id') id: string, @Body() updateCutiDto: UpdateCutiDto, @Req() req,): Promise<Cuti> {
        if (req.user.role === "Admin") {
            const updatedCuti = await this.cutiService.updateApproved(id, updateCutiDto,);
            return updatedCuti;
        } else {
